...
//		TableResult fileSink = tEnv.executeSql("insert into fs_table SELECT idid, sum(id) as id FROM dwx_product_source group by idid");
		Table tableselect = tEnv.sqlQuery("SELECT JSON_VALUE(json, path1) AS `idid`, TUMBLE_START(user_action_time, INTERVAL '1' MINUTE) as startTime,sum(id) as sumId FROM dwx_product_source  group  by idid, TUMBLE(user_action_time, INTERVAL '1' MINUTE)");
//		fileSink.print();
